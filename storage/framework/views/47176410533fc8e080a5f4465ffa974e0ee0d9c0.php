...
jQuery(document).on('click', '#apply_options_btn', function(e){				
	...
	jQuery.ajax({
		...
		success: function (res) {
			...
			var filter_total_record = jQuery('#filter_total_record').val();
			jQuery('.showing_total_record').html(filter_total_record);
			...
			//alert(record_limit);
			var showing_record = page_number*record_limit;
			if(filter_total_record<showing_record){
				jQuery('.showing_record').html(filter_total_record);
			}else{
				jQuery('.showing_record').html(showing_record);
			}
		},
